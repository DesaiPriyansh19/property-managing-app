...
class WalletPropertyAPI {
  ...
  static async createWalletProperty(propertyData) {
    try {
      ...
      if (propertyData.village) formData.append("village", propertyData.village)
      ...
      if (propertyData.serNoNew) formData.append("serNoNew", propertyData.serNoNew)
      if (propertyData.serNoOld) formData.append("serNoOld", propertyData.serNoOld)
      ...
      if (propertyData.mtrRoad) formData.append("mtrRoad", propertyData.mtrRoad)
      if (propertyData.nearByLandmark) formData.append("nearByLandmark", propertyData.nearByLandmark)
      if (propertyData.notes) formData.append("notes", propertyData.notes)
      if (propertyData.mapLink) formData.append("mapLink", propertyData.mapLink)
      ...
      throw new Error(error.response?.data?.message || "Failed to create wallet property")
    }
  }
  ...
  static async getAllWalletProperties(params = {}) {
    try {
      ...
      throw new Error(error.response?.data?.message || "Failed to fetch wallet properties")
    }
  }
  ...
  static async getWalletPropertyById(id) {
    try {
      ...
      throw new Error(error.response?.data?.message || "Failed to fetch wallet property")
    }
  }
  ...
  static async updateWalletProperty(id, propertyData) {
    try {
      ...
      Object.keys(propertyData).forEach((key) => {
        if (key !== "images" && key !== "pdfs" && propertyData[key] !== undefined && propertyData[key] !== "") {
      ...
      throw new Error(error.response?.data?.message || "Failed to update wallet property")
    }
  }
  ...
  static async deleteWalletProperty(id) {
    try {
      ...
      throw new Error(error.response?.data?.message || "Failed to delete wallet property")
    }
  }
  ...
  static async deleteWalletPropertyFile(propertyId, fileType, publicId) {
    try {
      ...
      const response = await api.delete(`/wallet-properties/${propertyId}/files/${fileType}/${encodedPublicId}`)
      return response.data
    } catch (error) {
